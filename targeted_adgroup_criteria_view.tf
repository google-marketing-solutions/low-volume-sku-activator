...
resource "google_bigquery_job" "adgroup_criteria_view" {
  depends_on = [google_bigquery_table.geo_targets_table]

  for_each = { for pair in var.accounts_table : pair.mc => pair }

  job_id = "adgroup_criteria_view_${each.value.gads}"

  labels = {
    "example-label" ="adgroup_criteria_view_${each.value.gads}"
  }
  ...
  query {
    query = <<EOF
        ...
        CREATE OR REPLACE VIEW `${var.gcp_project}.${var.zombies_dataset_name}.adgroup_criteria_view_${each.value.gads}`
        AS (
        WITH
            ...
            FROM
                `${var.gcp_merchant_and_gads_dataset_project}.${var.gads_dataset_name}.ads_Campaign_${each.value.gads}` AS Campaigns
            INNER JOIN
                `${var.gcp_merchant_and_gads_dataset_project}.${var.gads_dataset_name}.ads_AdGroup_${each.value.gads}` AS AdGroups
                USING (campaign_id, _DATA_DATE, _LATEST_DATE)
            INNER JOIN
                `${var.gcp_merchant_and_gads_dataset_project}.${var.gads_dataset_name}.ads_AdGroupCriterion_${each.value.gads}` AS AdGroupCriteria
